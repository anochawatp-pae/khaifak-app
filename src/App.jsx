...
function AnalysisTab({ history, setHistory }) {
  ...
  const runSearch = async () => {
    ...
    try {
      const prompt = `ช่วยค้นหาที่ดินขายจริงในประเทศไทยที่ตรงกับเกณฑ์ต่อไปนี้ จากเว็บประกาศขายที่ดิน (เช่น DDproperty, Kaidee, Baania, FazWaz หรือเว็บอื่นที่น่าเชื่อถือ):
...
หาแปลงจริง 3-5 แปลงที่ใกล้เคียงเกณฑ์ที่สุด ตอบเป็น JSON ล้วนเท่านั้น ห้ามข้อความอื่น ห้าม markdown code fence โครงสร้าง:
{"candidates":[{"title":"...","location":"...","size":"...","price":"...","deedType":"...","source_url":"...","highlight":"จุดเด่นสั้นๆ"}]}`;
      const data = await callClaude({
        system: "คุณคือผู้ช่วยสืบค้นประกาศขายที่ดินจริงในประเทศไทยสำหรับธุรกิจขายฝาก ตอบเป็น JSON เท่านั้น",
...
function RecommendedTab({ history, setHistory }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      const searchPrompt = `ช่วยค้นหาที่ดินขายจริงในประเทศไทยที่ตรงกับเกณฑ์ต่อไปนี้ จากเว็บประกาศขายที่ดิน (เช่น DDproperty, Kaidee, Baania, FazWaz หรือเว็บอื่นที่น่าเชื่อถือ) เน้นแปลงที่ดูมีศักยภาพสูงสำหรับธุรกิจขายฝาก:
...
หาแปลงจริง 4 แปลงที่น่าสนใจที่สุด ตอบเป็น JSON ล้วนเท่านั้น ห้ามข้อความอื่น ห้าม markdown code fence โครงสร้าง:
{"candidates":[{"title":"...","location":"...","size":"...","price":"...","deedType":"...","source_url":"...","highlight":"จุดเด่นสั้นๆ"}]}`;
